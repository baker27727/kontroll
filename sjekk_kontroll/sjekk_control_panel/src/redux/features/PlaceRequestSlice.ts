...
export const getAllPlaceRequests = createAsyncThunk('partners/place_requests', async () => {
    try {
...
export const approvePlaceRequest = createAsyncThunk('partners/place_creation_request_confirm', async (request_id: number) => {
    try {
      const response = await axios.post(`${baseUrl}/place-requests/${request_id}/approve`);
      return {
          status_code: response.status,
          data: response.data
      }
...
const PlaceRequestSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deletePlaceRequest.fulfilled, (state, action) => {
      state.loading = false;
      state.error = null;
      state.requests = state.requests.filter(request => request.id !== action.payload.data.id)
    });
    ...
    builder.addCase(approvePlaceRequest.pending, (state) => {
      state.loading = true;
    });
