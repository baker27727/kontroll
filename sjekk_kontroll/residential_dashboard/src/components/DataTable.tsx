...
export function DataTable<T>({
  ...
  const getValue = (obj, key: KeyType) => {
  ...
  return (
    <div className="overflow-x-auto border-gray-300 sm:rounded border">
          ...
          <tr>
            ...
            {columns.map((column) => (
              <th
                key={Array.isArray(column.key) ? column.key.join('.') : column.key}
                scope="col"
                className="px-6 py-3 bg-slate- border-b"
            ...
            {actions && <th scope="col" className="px-6 py-3 bg-slate- border-b text-gray-400 text-md">Actions</th>}
          </tr>
        </thead>
        <tbody>
          {loading ? (
