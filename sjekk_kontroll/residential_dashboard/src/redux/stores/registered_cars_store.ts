...
import RegisteredCarState from "../../interfaces/states/RegisteredCarState";
...
export const getRegisteredCars = createAsyncThunk(
    ...
    async (id: number) => {
        try{
            const response = await axiosClient.get(`residential-quarters/${id}/residential-cars`)
...
export const registeredCarSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(createResidentialCar.fulfilled, (state, action) => {
            state.loading = false
            state.error = null
            state.residential_cars.push(action.payload)
        })
