...
interface NotificationSubscriptionPayload {
    public_place_dashboard_id: number
}
...
class NotificationService {
    ...
    static listenOnNotificationPermissionChange = (public_place_dashboard_id: number) => {
        navigator.permissions.query({ name: 'notifications' }).then((notificationPerm) => {
            notificationPerm.onchange = async () => {
                if(notificationPerm.state == 'denied'){
                    await this.unsubscribe(public_place_dashboard_id)
                }

                if(notificationPerm.state == 'granted'){
                    await this.saveSubscription({
                        public_place_dashboard_id
                    })
                }
            };
        });
    ...
    static async saveSubscription(payload: NotificationSubscriptionPayload) {
        try{
            await axiosHttp.post(`/public-place-dashboard/${payload.public_place_dashboard_id}/notification/subscription`, {
    ...
    static async unsubscribe(public_place_dashboard_id: number) {
        try{
            await axiosHttp.post(`/public-place-dashboard/${public_place_dashboard_id}/notification/unsubscribe`)
        }catch(error) {
            console.log(error)
        }
    }
