...
export function DataTable<T>({
  ...
  const renderRow = (row: T, index: number, depth: number = 0) => {
    ...
    return (
      <React.Fragment key={index}>
        <tr
          className={`bg-white border-b-[2px] border-b-gray-100 ${
  ...
  return (
    <div className="border-gray-300 sm:rounded custom-scroll">
      <table className="w-full text-sm text-left text-gray-500">
        <thead className="text-xs text-gray-700 uppercase bg-gray-100">
          <tr>
            {getSubRows && <th className="px-4 py-3 bg-slate-100 border-b"></th>}
            {columns.map((column) => (
              <th
                key={Array.isArray(column.key) ? column.key.join('.') : column.key}
                scope="col"
                className="px-6 py-3 bg-slate-100 border-b"
            ...
            {actions && <th scope="col" className="px-6 py-3 bg-slate-100 border-b text-gray-400 text-md">Actions</th>}
          </tr>
        </thead>
        <tbody>
          {loading ? (
